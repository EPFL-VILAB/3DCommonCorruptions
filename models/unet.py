...
from models.models import TrainableModel
...
class UNet_merger_block(nn.Module):
    def __init__(self, input_channel, output_channel, down_size=False):
        super().__init__()
        self.conv1 = nn.Conv2d(input_channel, 16, 3, padding=1)
        self.bn1 = nn.GroupNorm(8, 16)
        self.conv2 = nn.Conv2d(16, 16, 3, padding=1)
        self.bn2 = nn.GroupNorm(8, 16)
        self.conv3 = nn.Conv2d(16, output_channel, 3, padding=1)
        #self.bn3 = nn.GroupNorm(2, output_channel)
        self.max_pool = nn.MaxPool2d(2, 2)
        self.relu = nn.ReLU()
        self.down_size = down_size
...
class UNet_Merger(TrainableModel):
    def __init__(self,  downsample=6, in_channels=3, out_channels=3):
        super().__init__()

        self.in_channels, self.out_channels, self.downsample = in_channels, out_channels, downsample
        self.merger1 = UNet_merger_block(2, 2, False)
        self.merger2 = UNet_merger_block(2, 2, False)
        self.merger3 = UNet_merger_block(2, 2, False)
