...
def get_blur_stack_single_image(rgb, blur_radii, cutoff_multiplier):
    ...
    #modules = [separable_gaussian for _ in args]
    #blurred_ims = parallel_apply([separable_gaussian]*len(args), args)
    #blurred_ims = parallel_apply(modules, args)
    #del modules
    #del args
    # just make everything serial
    blurred_ims = []
    for i in range(len(args)):
        #blurred_im_curr = separable_gaussian(args[i])
        blurred_im_curr = separable_gaussian( (args[i])[0], (args[i])[1],  (args[i])[2] )
        blurred_ims.append(blurred_im_curr)        
...
def get_blur_stack(rgb, blur_radii, cutoff_multiplier=None):
    ...
    outputs = []
    for i in range(len(args)):
        #out_curr = get_blur_stack_single_image(args[i])
        out_curr = get_blur_stack_single_image( (args[i])[0], (args[i])[1],  (args[i])[2] )
        outputs.append(out_curr)
