...
from models.midas.dpt_depth import DPTNormalModel
...
class ConsistentNormal(pl.LightningModule):
        ...
        if self.pretrained_weights_path is not None:
            ...
            if 'state_dict' in checkpoint:
                ...
                for k, v in checkpoint['state_dict'].items():
                    state_dict[k.replace('model.pretrained', 'pretrained')] = v
                    state_dict[k.replace('model.scratch', 'scratch')] = v
                    #state_dict[k] = v
            else:
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            '--replica_root', type=str, default='/scratch-data/ainaz/replica-taskonomized',
            help='Root directory of Replica dataset')
        parser.add_argument(
            '--gso_root', type=str, default='/scratch-data/ainaz/replica-google-objects',
            help='Root directory of GSO dataset.')
        parser.add_argument(
            '--hypersim_root', type=str, default='/scratch-data/ainaz/hypersim-dataset2/evermotion/scenes',
            help='Root directory of hypersim dataset.')
        parser.add_argument(
            '--blendedMVS_root', type=str, default='/scratch-data/ainaz/BlendedMVS/mvs_low_res_taskonomized',
    ...
    def setup_datasets(self):
        ...
        tasks = ['rgb', 'normal', 'mask_valid', 'reshading','depth_euclidean']        
    ...
    def select_val_samples_for_datasets(self):
        ...
        while len(val_imgs['blendedMVS']) < 20:
    ...
    def log_validation_example_images(self, num_images=20):
        ...
        for dataset in self.val_datasets:
            for img_idx in self.val_samples[dataset]:
                ...
                rgb_pos = rgb_pos.squeeze(0).unsqueeze(axis=0)
...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        verbose=True, monitor='val_normal_loss', mode='min', period=1, save_last=True, save_top_k=4
    )
    ...
    if args.restore is None:
        ...
        checkpoint_path = os.path.join(f'/scratch-data/oguzhan/train_omnidata_dpt_2d3daug/experiments/normal/checkpoints/{wandb_logger.name}/{args.restore}/last.ckpt')
        trainer = Trainer(
