...
def process_load(npyImage, objSettings, depth_loaded=None):
	...
	if use_pred:
		...
		#tenDepth = torch.from_numpy(depth_GT).to(dtype=torch.float32).cuda().unsqueeze(0).unsqueeze(0)
		#tenDepth = tenDepth / (2 ** 16 - 1.0)
		#tenDepth = nrmlz(tenDepth)
		#tenDepth[tenDepth >= 1.0] = tenDepth[tenDepth < 1.0].max()

		tenDepth = depth_GT.cuda().unsqueeze(0).unsqueeze(0)

		my_disparity = 1.0 / (tenDepth + 0.0000001)

		#my_disparity = disparity_adjustment(tenImage, my_disparity)
		#my_disparity = disparity_refinement(tenImage, my_disparity)
...
def process_kenburns(objSettings):
	...
	if 'boolInpaint' not in objSettings or objSettings['boolInpaint'] == True:
		...
		#for fltStep in [ 0.0, 0.5, 1.0 ]: #more inpainting locations
		for fltStep in [ 0.0, 0.25, 0.50, 0.75, 1.0 ]: #more inpainting locations
		#for fltStep in [ 0.0, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.0 ]: #even more inpainting locations	
			fltFrom = 1.0 - fltStep
			fltTo = 1.0 - fltFrom
